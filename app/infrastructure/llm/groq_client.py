...
class GroqClient(LlmClient):
    """
    Groq-backed implementation of the LLM client port.

    This adapter uses Groq's OpenAI-compatible Responses API via the official
    OpenAI Python client configured with Groq's base URL.
    ...
    def __init__(
        ...
        *,
        ...
        if not normalized_api_key:
            msg = "Groq API key must not be empty."
            raise ValueError(msg)

        if not normalized_model:
            msg = "Groq model must not be empty."
            raise ValueError(msg)

        if not normalized_base_url:
            msg = "Groq base URL must not be empty."
            raise ValueError(msg)
    ...
    async def generate_checklist(
        ...
        """
        Generate a structured checklist for the selected task using Groq.
        ...
        try:
            ...
            msg = f"Failed to communicate with Groq: {exc!s}"
            raise LlmProviderError(msg) from exc
        except Exception as exc:
            msg = f"Unexpected Groq client error: {exc!s}"
            raise LlmProviderError(msg) from exc
    ...
    def _extract_output_text(self, response: object) -> str:
        ...
        msg = "Groq response did not contain textual output."
